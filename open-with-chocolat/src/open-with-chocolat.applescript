on alfred_script(q)
    ...
    if (q as string) is "" then
        ...
        try
            ...
            tell application "Finder"
                open theFile using appPath
            end tell


        on error

            -- if q does not resolve to complete file paths, test whether the file exists in the current target directory.
            -- if not, create it
            -- open it			
            ...
            try
                tell application "Finder"
                    ...
                    set theFile to theTarget as string & q

                    try
                        theFile as alias
                    on error
                        set newDoc to make new file at theTarget with properties {name:q}
                    end try
